...
6. **Schema changes in this project are additive-only. No destructive changes — ever.** This means:
   - **Never drop a column** from any table in `lib/db/src/schema/*`. If a field becomes unused, leave the column in place (mark it deprecated in a comment) and just stop reading/writing it from the API and UI.
   - **Never drop a table.** If a feature is removed, leave the table; stop using it.
   - **Never rename a column or table** in dev without coordinating a Publish-time rename confirmation. The safe default is: add a new column / new table, dual-write, then deprecate the old one (without dropping it).
   - **Never tighten a column** (NOT NULL added, type narrowed, default removed) on a table that has prod data.
   - The reason: when the user clicks Publish, Replit diffs dev vs prod and applies the diff to prod. If the diff contains a drop/rename, the validator rejects it and the **only** UI option Replit offers to "resolve" it is **"Copy your development database schema & data to production"**, which despite its name **wipes every row in prod** — including the runtime-only tables (`approval_decisions`, `conversations`, `messages`, `share_post_feedback`, and any future client-feedback table) that the bootstrap deliberately never touches because they only exist in production. Clicking that dialog destroys real client data.
...
9. **After creating a new page file under `src/pages/`, always register it in `artifacts/virtu-ferries-brand-hub/src/App.tsx` in the same session.** Import the component at the top of the file alongside the other page imports, then add a `<Route path="/your-path" component={YourPage} />` inside the `<Switch>` in `BrandedRoutes` (or at the top-level Switch for hub-level pages like `/nico`). Forgetting this leaves Vite unable to resolve the import and causes a runtime 404 for the new route.
