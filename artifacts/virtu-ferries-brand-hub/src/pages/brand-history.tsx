...
export default function BrandHistory() {
  ...
  return (
    ...
    >
      {/* Hero */}
      <div className="relative overflow-hidden border-b border-[#F4F4F5] bg-gradient-to-b from-white to-gray-50/40">
        ...
        <div className="relative max-w-5xl mx-auto px-6 md:px-10 py-12 md:py-16">
          <motion.div {...fadeUp(0)} className="space-y-4 max-w-3xl">
            ...
            <h1 className="h-display text-[#18181B] max-w-2xl">
              {history.hero.title}
            </h1>
            <p className="text-[15px] text-[#71717A] font-light leading-relaxed max-w-xl">
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-10 space-y-14 pt-12">
          ...
          <motion.div {...fadeUp(0.05)}>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {history.stats.map((s) => {
                ...
                return (
                  <div key={s.label} className="surface-card p-5 space-y-3 hover:border-[#E4E4E7]/80 transition-colors">
                    <div className="h-8 w-8 rounded-lg grid place-items-center" style={{ background: `${s.color}12` }}>
                      <Icon className="w-4 h-4" style={{ color: s.color }} />
                    </div>
                    <p className="text-2xl font-semibold tracking-[-0.02em] text-[#18181B] tabular-nums">{s.value}</p>
                    <p className="text-[10px] text-[#A1A1AA] leading-relaxed uppercase tracking-[0.16em] font-medium">{s.label}</p>
                  </div>
                );
              })}
            </div>
        ...
        {history.timeline.length > 0 && (
          <motion.section {...fadeUp(0.1)} className="space-y-5">
            <h2 className="h-section text-[#18181B] flex items-center gap-2.5">
              <span className="accent-dot bg-[var(--brand-primary)]" />
              The story so far
            </h2>

            <div className="relative">
              <div className="absolute left-[19px] top-0 bottom-0 w-px bg-[#F4F4F5] md:left-[23px]" />
              ...
              <div className="space-y-8">
                {history.timeline.map((item, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 surface-card p-5 space-y-1.5 hover:border-[#E4E4E7]/80 transition-colors">
                      <div className="flex items-baseline gap-3 flex-wrap">
                        <span className="text-[10px] font-medium uppercase tracking-[0.18em]" style={{ color: item.accent }}>
                          {item.year}
                        </span>
                        <h3 className="h-card text-[#18181B]">{item.title}</h3>
                      </div>
                      <p className="text-sm text-[#52525B] leading-relaxed font-light">{item.body}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
          ...
          <motion.section {...fadeUp(0.15)} className="space-y-5">
            <div className="space-y-1.5">
              ...
              {history.fleetSubtitle && (
                <p className="text-[13px] text-[#A1A1AA] ml-4">{history.fleetSubtitle}</p>
              )}
            </div>

            <div className="grid md:grid-cols-2 gap-4">
              {history.vessels.map((v) => (
                <div key={v.name} className="surface-card overflow-hidden hover:border-[#E4E4E7]/80 transition-colors">
                  <div className="h-[3px]" style={{ backgroundColor: v.accent }} />
                  <div className="p-6 space-y-5">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2 mb-1.5">
                          <Ship className="w-3.5 h-3.5 text-[#A1A1AA]" />
                          <span className="text-[10px] font-medium uppercase tracking-[0.18em] text-[#A1A1AA]">{v.role}</span>
                        </div>
                        <h3 className="text-lg font-semibold tracking-[-0.01em] text-[#18181B]">{v.name}</h3>
                      </div>
                      <span
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        ...
                        <div key={spec.label} className="bg-[#F5F5F5]/70 rounded-lg p-2.5">
                          <p className="text-[9px] text-[#A1A1AA] uppercase tracking-[0.14em] mb-0.5 font-medium">{spec.label}</p>
                          <p className="text-[13px] font-semibold text-[#18181B] tabular-nums">{spec.value}</p>
                        </div>
                      ))}
                    </div>

                    <p className="text-sm text-[#52525B] leading-relaxed font-light">{v.description}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <motion.section {...fadeUp(0.2)} className="space-y-5">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {history.heritage.map((item) => {
                ...
                return (
                  <div key={item.title} className="surface-card p-5 space-y-3 hover:border-[#E4E4E7]/80 transition-colors">
          ...
          <motion.section {...fadeUp(0.22)} className="space-y-5">
            ...
            <div className="surface-card p-7 flex flex-col md:flex-row gap-8">
              <div className="space-y-2.5 flex-1">
                ...
                <h3 className="text-base font-semibold tracking-[-0.01em] text-[#18181B]">{history.sister.title}</h3>
                <p className="text-sm text-[#52525B] leading-relaxed font-light">{history.sister.description}</p>
              </div>
              <div className="flex flex-col gap-2.5 shrink-0 md:border-l md:border-[#F4F4F5] md:pl-8">
                {history.sister.details.map((d) => (
                  <div key={d.label} className="flex items-center gap-4">
                    <span className="text-[11px] text-[#A1A1AA] w-24 shrink-0 uppercase tracking-wider font-medium">{d.label}</span>
                    <span className="text-[13px] font-semibold text-[#18181B] tabular-nums">{d.value}</span>
                  </div>
                ))}
              </div>
            </div>
