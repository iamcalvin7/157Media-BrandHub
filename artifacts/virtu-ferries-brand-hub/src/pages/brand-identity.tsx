  ...
  return (
    ...
    >
      <header className="space-y-5 pb-2">
        ...
        <h1 className="h-display text-[#18181B]">Brand Identity</h1>
        <p className="text-lg text-[#71717A] font-light max-w-2xl leading-relaxed">{identity.headerSubtitle}</p>
      ...
      {identity.brandStory && (
        <section className="space-y-6">
          <h2 className="h-section text-[#18181B] flex items-center gap-3">
            <span className="accent-bar bg-[var(--brand-primary)]" />
            Brand Story
          </h2>
          <div className="p-8 bg-white border border-[#F4F4F5] rounded-2xl relative overflow-hidden">
            <div className="absolute -top-24 -right-24 w-64 h-64 bg-[var(--brand-primary)]/5 rounded-full blur-3xl pointer-events-none"></div>
            <p className="text-lg text-[#3F3F46] leading-relaxed font-light">{identity.brandStory}</p>
          </div>
        </section>
      )}
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {identity.toneOfVoice.length > 0 && (
            <section className="space-y-6">
              ...
              <div className="h-full p-8 bg-white border border-[#F4F4F5] rounded-2xl space-y-4">
                {identity.toneOfVoice.map((line, i) => (
                  <p key={i} className="text-[#3F3F46] leading-relaxed font-light">{line}</p>
                ))}
              </div>
            </section>
          )}
          ...
          {identity.keyMessages.length > 0 && (
            <section className="space-y-6">
              ...
              <div className="h-full p-8 bg-white border border-[#F4F4F5] rounded-2xl">
                <ul className="space-y-4">
                  {identity.keyMessages.map((msg, i) => (
                    <li key={i} className="flex items-start gap-3 text-[#3F3F46] font-light">
        ...
        <section className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {identity.whatToSay.length > 0 && (
              <div className="p-6 bg-white border border-[#F4F4F5] rounded-2xl border-l-4 border-l-[var(--brand-primary)]">
                <h3 className="h-card mb-5 flex items-center gap-2 text-[#18181B]">
                  <CheckCircle2 className="w-5 h-5 text-[var(--brand-primary)]" />
                  What to Say
                </h3>
                <ul className="space-y-3">
                  {identity.whatToSay.map((item, i) => (
                    <li key={i} className="text-[#52525B] font-light text-sm leading-relaxed">{item}</li>
                  ))}
                </ul>
              </div>
            )}
            ...
            {identity.whatNotToSay.length > 0 && (
              <div className="p-6 bg-white border border-[#F4F4F5] rounded-2xl border-l-4 border-l-[var(--brand-alert)]">
                <h3 className="h-card mb-5 flex items-center gap-2 text-[#18181B]">
                ...
                <ul className="space-y-3">
                  {identity.whatNotToSay.map((item, i) => (
                    <li key={i} className="text-[#52525B] font-light text-sm leading-relaxed line-through decoration-white/20">{item}</li>
                  ))}
                </ul>
              </div>
            )}
