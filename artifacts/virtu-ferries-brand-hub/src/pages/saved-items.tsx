...
const KINDS: { value: Kind | "all"; label: string; icon: any; color: string }[] = [
  { value: "all", label: "All", icon: Bookmark, color: "text-gray-600" },
...
function SavedCard({ item, onDelete }: { item: SavedItem; onDelete: () => void }) {
  ...
  return (
    <motion.div
      ...
      className="group bg-white rounded-2xl border border-gray-200 hover:border-gray-300 hover:shadow-md transition-all flex flex-col overflow-hidden"
    >
      {item.thumbnailUrl && (
        <a href={item.url ?? "#"} target="_blank" rel="noreferrer" className="block aspect-video bg-gray-100 overflow-hidden">
      ...
      <div className="p-4 flex-1 flex flex-col">
        <div className="flex items-center gap-2 mb-2">
          <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">
            <Icon className={cn("w-3 h-3", meta.color)} />
            {item.kind}
          </span>
          {host && (
            <span className="text-[11px] text-gray-400 truncate">{host}</span>
          )}
        </div>
        ...
        {item.notes && (
          <p className="mt-2 text-xs text-gray-600 leading-relaxed line-clamp-3 whitespace-pre-wrap">
            {item.notes}
          </p>
        )}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Save for later</h2>
            <p className="text-xs text-gray-500 mt-0.5">A link, a video reference or a design idea.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <label className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold mb-1.5 block">URL</label>
            <input
              type="url"
              value={url}
          ...
          <div>
            <label className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold mb-1.5 block">Title</label>
            <input
              value={title}
          ...
          <div>
            <label className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold mb-1.5 block">Notes</label>
            <textarea
              value={notes}
          ...
          <div>
            <label className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold mb-1.5 block">Thumbnail URL <span className="normal-case text-gray-300 font-normal">(optional)</span></label>
            <input
              type="url"
              value={thumbnailUrl}
