// Per-brand static content registry.
//
// Every page that used to hardcode Virtu Ferries copy now reads its slice from
// here, indexed by the active brand's slug. Virtu Ferries is fully populated
// (carried over verbatim from the original hardcoded pages). Gozo Highspeed is
// scaffolded with empty arrays / placeholder values so the team can fill it in
// over time without code changes here being a blocker — when a section is
// empty, the page renders an "Not configured yet" empty state instead of
// crashing or silently leaking the other brand's content.
